        ...
        if len(approx) == 4:
            ...
            for i in range(4):
                ...
                if abs(angle_sum - 180) < angle_threshold:
                    ...
                    break
            print("筛选has_parallel_sides", has_parallel_sides)
            ...
            x, y, w, h = cv2.boundingRect(approx)
            aspect_ratio = w / float(h)
            print("筛选aspect_ratio", aspect_ratio)
            # 允许±10%的误差
...
def get_square_num(frame_cut, cnt, output_size=200):
     # 1. 创建mask
    mask = np.zeros(frame_cut.shape[:2], dtype=np.uint8)
    ...
    roi = frame_cut[y:y+h, x:x+w]
    mask_roi = mask[y:y+h, x:x+w]

    # 3. 创建红色背景
    red_bg = np.full((h, w, 3), (50, 50, 50), dtype=np.uint8)

    # 4. 把目标区域贴到红色背景
    fg = cv2.bitwise_and(roi, roi, mask=mask_roi)
    bg = cv2.bitwise_and(red_bg, red_bg, mask=cv2.bitwise_not(mask_roi))
    ...
    # 5. resize到指定大小
    output = cv2.resize(combined, (output_size, output_size), interpolation=cv2.INTER_LINEAR)
